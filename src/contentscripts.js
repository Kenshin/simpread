...
function focusMode() {
    ...
    watch.Verify( result => {
        if ( result.site ) {
            new Notify().Render( "适配列表已更新，刷新当前页面后才能生效。", "刷新", ()=>window.location.reload() );
        } else {
...
function readMode() {
    ...
    watch.Verify( result => {
        if ( result.site ) {
            new Notify().Render( "适配列表已更新，刷新当前页面后才能生效。", "刷新", ()=>location.href = location.href + "?simpread_mode=read" );
        } else {
            getCurrent( mode.read );
