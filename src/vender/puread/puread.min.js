(function (global, factory) {
  ...
  var AdapteSite = function () {
      ...
      createClass(AdapteSite, [{
          ...
          key: "isMathJax",
          value: function isMathJax() {
              var _this = this;

              if (this.mathjax == undefined) {
                  this.mathjax = false;
                  $('body').find('script').each(function (idx, item) {
                      item.type.startsWith('math') && (_this.mathjax = true);
                  });
              }
              return this.mathjax;
          }
           ...
           * MathJax View
           * 
           * @return {object} return type include: jquery; string e.g. <div id="xxxx"> ;undefined
           */

      }, {
          key: "MathJaxMode",
          value: function MathJaxMode() {
              var $dom = readtmpl();
              if ($dom != -1) {
                  ...
                  var article = readmozilla();
                  if (article && article.content != "") {
                      var a1 = article.content.replace('<div id="readability-page-1" class="page">', ''),
                          a2 = $(a1)[0],
                          a3 = a2.outerHTML.replace(a2.innerHTML, ""),
                          a4 = $(a3)[0],
                          tag = a4.tagName.toLowerCase(),
                          cls = a4.className,
                          id = a4.id;
                      if (id != "") {
                          return "<" + tag + " id=\"" + id + "\">";
              ...
              if (meta) {
                  ...
                  if (matching.length > 0) {
                      ...
                      matching.forEach(function (site) {
                          if (site[1].active) {
                              found = site;
                              _this2.current.url = found[0];
                              _this2.current.site = _this2.Safesite(_extends({}, found[1]), found[2], found[0]);
                              _this2.state = "adapter";
                          }
                      });
                      if (!found) {
                      ...
                      console.log('%c Current parse is Adapter', 'background:#2196f3; color: #fff');
                  } else {
                      var obj = readmulti();
                      if (obj != -1) {
  ...
  function readmeta() {
      ...
      if ($($("body").children()[0]).is('pre') && (minimatch(location.href, "file://**/*.md") || minimatch(location.href, "http*://**/*.md"))) {
          return readmd();
      }
      ...
      if (meta.name && meta.include) {
          ...
          idx == -1 && console.log('%c Current parse is Meta', 'background:#2196f3; color: #fff', meta);
  ...
  function readtxt() {
      ...
      console.log('%c Current parse is TXT', 'background:#2196f3; color: #fff', meta);
      return meta;
  }
  ...
  function readmd() {
      ...
      console.log('%c Current parse is Markdown', 'background:#2196f3; color: #fff', meta);
      return meta;
  }
  ...
  function readtmpl() {
      var $root = $("body"),
          selectors = ["[itemprop='articleBody']", "article", ".post-content", ".entry-content", ".post-article", ".content-post", ".article-entry", ".article-content", ".article-body", ".markdown-body", ".post", ".content"];
      ...
      try {
          for (var _iterator2 = selectors[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
              ...
              if ($target.length > 0) {
                  console.log('%c Current parse is SimpRead', 'background:#2196f3; color: #fff', selector$$1);
                  return $target;
              }
          }
      } catch (err) {
  ...
  /**
   * Read mode usage mozilla
   * 
   * @return {object} article
   */
  function readmozilla() {
      ...
      console.log('%c Current parse is Mozilla', 'background:#2196f3; color: #fff', article);
      return article;
  }
  ...
  var PureRead = function (_AdapteSite) {
      ...
      function PureRead(sites) {
          ...
          _this.version = "0.0.4 build 0427";
      ...
      createClass(PureRead, [{
           ...
           * Get Dom
           * 
           * @param {string} html, e.g. <div id="xxxx">
           * @param {string} type, @see query
           */

      }, {
          key: 'GetDom',
          value: function GetDom(str, type) {
              var include = selector(str),
                  dom = query(include, type);
              return dom;
          }
          ...
          value: function Beautify($target) {
              if (this.plugin.beautify) {
                  this.cleanup && this.plugin.beautify.cleanHTML($target, this.pure, this.isMathJax());
